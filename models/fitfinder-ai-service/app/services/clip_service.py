...
# clip_service.py
import uuid
from app.workers.clip_worker.worker import process_clip_task  # direct call instead of Celery

VALID_ITEM_TYPES = ["uploaded_img", "store_item"]

def enqueue_clip_task(image_data: str, callback_url: str, item_type: str = "uploaded_img"):
    """
    Validates input and directly processes the CLIP task (Celery disabled).
    """
    if not image_data:
        raise ValueError("Image data is required")
    if item_type not in VALID_ITEM_TYPES:
